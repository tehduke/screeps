...
function getSupplyTask (roomName, creep) {
	...
	//cull tasks from local creep task list to stop oversubscti
 	if (DEBUG.haulerVerbose) {
		console.log("========")
...
function releaseTask (roomName, structId, creep) {
	...
	let stask = Game.rooms[roomName].supplyTasks
	let rtask = Game.rooms[roomName].requestTasks
	let tasks = stask.concat(rtask)
	let task = _.filter(tasks, (t) => t.structureId === structId);
	//hackworkaround for if task has been culled from valid task list
	if (task == undefined) {
		let structure = Game.getObjectById(structId);
		if (structure !== null) {
